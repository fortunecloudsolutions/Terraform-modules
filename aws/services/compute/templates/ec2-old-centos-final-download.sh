...
sudo chmod +x $LOCAL_DIR/final-java.sh
touch $LOCAL_DIR/log.txt
cd $LOCAL_DIR
pwd >> $LOCAL_DIR/log.txt
sudo ./final-java.sh -a ${java_dist} -b ${jdk_version} -c ${default_java_dir} -d ${java_download_dir} >> $LOCAL_DIR/log.txt #jdk-8u202-linux-x64.tar.gz jdk1.8.0_202 /usr/lib/jvm /home/ec2-user/